	...
	floorTiles_(spriteRepository().getSheet(SpriteSheets::Floor), floorScale)
{
	...
	insertObject(std::make_shared<CustomerObject>(getRandomNum(), vec2(50.0f), customerTypeRepository().get(CustomerTypes::Penguin), itemRepository().get(Items::SlipperyOrb)));
