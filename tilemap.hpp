...
public:
	Tilemap(std::shared_ptr<SpriteSheet> sheet, float scale = 1.0f) :
		spriteSheet_(sheet),
		scale_(scale),
		tileSize_(Tmpl8::vec2(sheet->getWidth(), sheet->getHeight()) * scale)
	{}

	void setTile(Tmpl8::vec2& pos, TilemapType tile) { tiles_[pos] = tile; }
	...
	void setSquare(Rect2<int>& rect, std::vector<TilemapType>& tiles){
		...
		for (int x = 0; x < rect.width; x++) {
			...
			for (int y = 0; y < rect.height; y++) {
				...
				setTile(Tmpl8::vec2(posX, posY), tiles[randomNum % tiles.size()]);
				posY += tileSize_.y;
			}
			posX += tileSize_.x;
		}
	...
	void draw(Tmpl8::Surface* surface, Tmpl8::vec2& offset) {
...
private:
	...
	Tmpl8::vec2 tileSize_;

	std::shared_ptr<SpriteSheet> spriteSheet_;
	std::map<Tmpl8::vec2, TilemapType> tiles_ = {};
};